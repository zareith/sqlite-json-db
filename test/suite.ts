...
export function Suite(Database: {new(): Base}) {
	describe("Query", () => {
		...
		it("supports updating records", () => {
			const db = new Database();
			db.setLogger(console)
			const users = db.collection<User>("users");
			users.put(null, {name: "A1", age: 15});
			users.put(null, {name: "B1", age: 10})
			users.put(null, {name: "C1", age: 20})
			users.where({
				name: {$in: ["A1", "B1", "C1"]}
			}).update({
				age: 5
			});
			const rows = users.where({
				name: {$in: ["A1", "B1", "C1"]}
			}).get();
			let i = 0;
			for (; i < rows.length; i++) {
		...
		it("supports deleting records", () => {
			...
			db.setLogger(console)
			const users = db.collection<User>("users");
			users.put(null, {name: "A2", age: 15});
			users.put(null, {name: "B2", age: 10})
			users.put(null, {name: "C2", age: 20})
			users.where({
				name: {$in: ["A2", "B2", "C2"]}
			}).delete();
			const count = users.where({
				name: {$in: ["A1", "B1", "C1"]}
			}).count();
			assert(count === 0);
			const rows = users.where({
