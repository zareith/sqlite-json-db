...
export function Suite(Database: { new(): Base }) {
    ...
    describe("query", () => {
        it("should query", async () => {
            ...
            await usersRef.doc().put({ username: "zareith", updatedAt: 234 });
        ...
        it("supports composite queries", async () => {
            ...
            await usersRef.doc().put({ name: "John", age: 10 });
            await usersRef.doc().put({ name: "Kennedy", age: 45 });
            await usersRef.doc().put({ name: "Sita", age: 20 });
    ...
    describe("docRef", () => {
        ...
        it("should get an old doc", async () => {
            ...
            await oldUsersRef.put({ username: "John Doe", updatedAt: 123123 });
            const usersRef = db.collection("users").doc("123");
            let user = await usersRef.get();
            assert.strictEqual(user?.username, "John Doe");
            await oldUsersRef.update({ username: "Jane Doe" });
